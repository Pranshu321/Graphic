...
function SidebarRight() {
  return (
    <div className="p-3 md:w-72 shrink-0 md:sticky md:top-16 h-full">
                ...
                <div className="w-1/2 p-3">
                  ...
                  <Countdown
                    ...
                    renderer={({ hours, minutes, seconds }) => (
                      <div className="text-red-600">{`${hours}h: ${minutes}m: ${seconds}s`}</div>
                    )}
                  />
                </div>
              </div> */}
  ...
  return (
    <div className="">
      ...
      <div className="p-3">
        ...
        >
          <h2 className="font-bold text-white text-3xl max-w-sm">
            Find a collection of best artwork here
          </h2>
          <button className="py-2 bg-gradient-to-tr from-fuchsia-600 to-violet-600 text-white rounded-md w-44 mt-3">
            Explore Now
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between px-3 mt-3">
        ...
        <div className="flex justify-center items-center space-x-3 ">
          ...
          <input
            className="relative m-0 block w-full min-w-0 flex-auto cursor-pointer rounded border border-solid border-neutral-300  bg-clip-padding py-[0.32rem] px-3 leading-[2.15] font-normal text-neutral-700 transition duration-300 ease-in-out file:-mx-3 file:-my-[0.32rem] file:cursor-pointer file:overflow-hidden file:rounded-none file:border-0 file:border-solid file:border-inherit file:px-3 file:py-[0.32rem] file:text-white file:bg-teal-600 file:transition file:duration-150 file:ease-in-out file:[margin-inline-end:0.75rem] file:[border-inline-end-width:1px] hover:file:bg-teal-400 focus:border-primary focus:text-neutral-700 focus:shadow-[0_0_0_1px] focus:shadow-primary focus:outline-none"
            id="formFileLg"
          ...
          <button
            onClick={uploadFile}
            className="bg-transparent hover:bg-greenish text-greenish font-semibold hover:text-white py-2 px-4 border border-greenish hover:border-transparent rounded"
          >
            Upload
          </button>
        </div>
...
function Header() {
  return (
    <div className="flex flex-wrap p-3 items-center sticky text-white top-0 bg-zinc-900 h-fit  md:h-16 z-30">
...
function SidebarItem({ text, index }) {
  return (
    <li className="relative">
      <Link
        to={`/${text.toLowerCase()}`}
        className={`pl-4 flex items-center capitalize text-zinc-500`}
      >
        <span
          className={`w-8 h-8 grid place-items-center mr-2 rounded-md bg-zinc-800`}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5"
